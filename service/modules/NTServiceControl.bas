...
Private Const SERVICE_WIN32_OWN_PROCESS = &H10& ''+ SERVICE_INTERACTIVE_PROCESS
...
Private Const SERVICE_DEMAND_START As Long = 2
...
Private Declare Function OpenService _
      ...
      (ByVal hSCManager As Long, ByVal lpServiceName As String, _
      ByVal dwDesiredAccess As Long) As Long   '** Change frmServiceControl.SERVICE_NAME as needed
...
If hSCManager <> 0 Then
    hService = OpenService(hSCManager, frmServiceControl.SERVICE_NAME, SERVICE_QUERY_STATUS)
    If hService <> 0 Then
...
hSCManager = OpenSCManager(vbNullString, vbNullString, _
                       SC_MANAGER_CONNECT)
If hSCManager <> 0 Then
    hService = OpenService(hSCManager, frmServiceControl.SERVICE_NAME, SERVICE_QUERY_CONFIG)
    If hService <> 0 Then
        ReDim SCfg(1 To 1)
...
Dim SERVICE_WIN32_vars As String
...
SERVICE_WIN32_vars = SERVICE_WIN32_OWN_PROCESS + SERVICE_INTERACTIVE_PROCESS
Else
SERVICE_WIN32_vars = SERVICE_WIN32_OWN_PROCESS
End If
...
If hSCManager <> 0 Then
    ...
    hService = CreateService(hSCManager, frmServiceControl.SERVICE_NAME, _
                       frmServiceControl.SERVICE_NAME, SERVICE_ALL_ACCESS, _
                       SERVICE_WIN32_vars, _
                       SERVICE_DEMAND_START, SERVICE_ERROR_NORMAL, _
                       frmServiceControl.Service_File_Name + " -service_start", vbNullString, _
...
hSCManager = OpenSCManager(vbNullString, vbNullString, _
                       SC_MANAGER_CONNECT)
If hSCManager <> 0 Then
    hService = OpenService(hSCManager, frmServiceControl.SERVICE_NAME, _
                       SERVICE_ALL_ACCESS)
    If hService <> 0 Then
...
hSCManager = OpenSCManager(vbNullString, vbNullString, _
                       SC_MANAGER_CONNECT)
If hSCManager <> 0 Then
    hService = OpenService(hSCManager, frmServiceControl.SERVICE_NAME, SERVICE_START)
    If hService <> 0 Then
...
hSCManager = OpenSCManager(vbNullString, vbNullString, _
                       SC_MANAGER_CONNECT)
If hSCManager <> 0 Then
    hService = OpenService(hSCManager, frmServiceControl.SERVICE_NAME, SERVICE_STOP)
    If hService <> 0 Then
